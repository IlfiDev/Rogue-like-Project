...
public class AstralStep : Abiility
{
    ...
    {
        if(user.GetComponent<Player>().UseMana(manaCost))
        {
    ...
    private void Step(Vector3 targetPosition, GameObject user)
    {
        ...
        {
            gameController.astralStepTargerPoint = Vector3.MoveTowards(user.transform.position, targetPosition, (float)radius);
            gameController.astralStepUser = user;
        }
    }
}